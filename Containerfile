...
# `bluebuild build recipes/recipe.yml` and the GitHub Actions workflow
...
#   - recipes/recipe.yml — source of truth for the assembled image
...
# -----------------------------------------------------------------------------
# Stage 2: identity. Mirrors recipes/base.yml step 1 (files).
...
# -----------------------------------------------------------------------------
COPY files/etc/os-release       /etc/os-release
COPY files/etc/orion-release    /etc/orion-release

# -----------------------------------------------------------------------------
# Stage 3: package layer. Mirrors recipes/base.yml step 2 and
# recipes/kde.yml step 1. We use rpm-ostree inside the image so
...
LABEL org.opencontainers.image.title="Orion OS" \
      ...
      io.orionos.recipe="recipes/recipe.yml"
